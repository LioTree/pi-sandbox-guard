...
describe("find tool sandbox integration", () => {
  it("does not leak files from denyRead paths when searching an allowed root", async () => {
    if (!SandboxManager.isSupportedPlatform()) {
      console.warn(`skip: sandbox-runtime backend is not supported on ${process.platform}`);
      return;
    }
    if (spawnSync("fd", ["--version"], { stdio: "ignore" }).status !== 0) {
      console.warn("skip: fd is not available");
      return;
    }

    const root = await mkdtemp(path.join(tmpdir(), "psg-srt-find-"));
    const publicDir = path.join(root, "public");
    const deniedDir = path.join(root, "denied");
    await mkdir(publicDir);
    await mkdir(deniedDir);
    await writeFile(path.join(publicDir, "visible.txt"), "visible", "utf-8");
    await writeFile(path.join(deniedDir, "secret.txt"), "secret", "utf-8");
    ...
    try {
      const tool = createFindTool(() => makeServices(config, { sandbox }));
      const text = toolText(
      ...
      expect(text).not.toContain("denied/secret.txt");
    } finally {
      await sandbox.reset();
    }
  });
